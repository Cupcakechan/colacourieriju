...
//
// This is a self-contained dev tool: its tunables live HERE (not in config.js) so the
// module can be removed in one step for the shipping build — delete the import + the
// two `editor` lines in main.js and this file, and nothing else changes.
...
export function createEditor({ canvas, camera, objects, map, types }) {
  ...
  function onDown(e) {
    ...
    const wx = sx + camera.cam.x, wy = sy + camera.cam.y;
    const hit = objects.hitTest(wx, wy);
    if (hit) {
      selected = hit;
    } else if (selectedType) {
      // place so the object's BASE (centered on anchorY) lands under the cursor
      const def = types[selectedType];
      selected = objects.add(selectedType, wx - def.w / 2, wy - def.anchorY);
      flash(`+${selectedType}`);
    } else {
      selected = null;
      return;
  ...
  function onMove(e) {
    ...
    const { sx, sy } = screenFromEvent(e);
    const wx = sx + camera.cam.x, wy = sy + camera.cam.y;
    objects.moveTo(selected, wx + dragDX, wy + dragDY);
  }
  ...
  function onKey(e) {
    if (e.code === "KeyE") { // toggle editor mode from anywhere
      active = !active;
      if (active) { camX = camera.cam.x + VW / 2; camY = camera.cam.y + VH / 2; flash("editor ON"); }
      dragging = false;
      e.preventDefault();
      return;
    }
    ...
    if (e.code === "Delete" || e.code === "Backspace") {
      if (selected) { objects.remove(selected); selected = null; flash("deleted"); }
      e.preventDefault();
    } else if (e.code === "KeyX") {
      exportPlacements();
      e.preventDefault();
  ...
  function exportPlacements() {
    const ps = objects.getPlacements();
  ...
  function update(dt, input) {
    ...
    camX = Math.max(VW / 2, Math.min(camX, Math.max(VW / 2, map.width - VW / 2)));
    camY = Math.max(VH / 2, Math.min(camY, Math.max(VH / 2, map.height - VH / 2)));
  }
  ...
  function draw(ctx) {
    ...
    if (selected) {
      ...
      if (def) {
        ...
        ctx.lineWidth = 2;
        ctx.strokeRect(
          Math.floor(selected.x - camera.cam.x) + 0.5,
          Math.floor(selected.y - camera.cam.y) + 0.5,
          def.w - 1, def.h - 1
        );
      }
    }
    ...
    for (let i = 0; i < typeNames.length; i++) {
      ...
      const rec = objects.imageFor(name);
      if (rec && rec.ready) {
    ...
    ctx.fillStyle = CONFIG.COLORS.text;
    ctx.fillText(`objects: ${objects.count}   sel: ${selected ? selected.type : "—"}`, pr.x + PANEL_PAD, pr.y + pr.h - 24);
    if (msgT > 0) {
